...
import { GameSession } from '../../models/quiz.models';
...
export class SessionControlComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.loadSession(id);
        // Poll toutes les 2s pour les joueurs et le statut
        this.pollInterval = setInterval(() => this.refreshData(), 2000);
    }
    ...
    private refreshData(): void {
        ...
        // Refresh session, then fetch players/scores based on fresh status
        this.sessionService.getSessionById(id).subscribe(s => {
            this.session = s;
            this.cdr.detectChanges();

            // quiz is stripped by @JsonBackReference — load via quizId
            ...
            if (s.status === 'WAITING') {
                this.sessionService.getPlayers(id).subscribe(p => {
                    this.players = p;
                    this.cdr.detectChanges();
                });
            }

            if (s.status === 'STARTED' || s.status === 'FINISHED') {
                this.sessionService.getScoresBySession(id).subscribe(scores => {
                    this.scores = scores.sort((a, b) => b.points - a.points);
                    this.cdr.detectChanges();
                });
                this.sessionService.getDetailedScores(id).subscribe(detail => {
    ...
    endGame(): void {
        if (!this.session?.id) return;
        this.http.patch(`/quiz-api/game-sessions/${this.session.id}/status?status=FINISHED`, {})
            .subscribe(() => { this.cdr.detectChanges(); this.refreshData(); });
    }

    private buildGrid(detail: any[]): void {
