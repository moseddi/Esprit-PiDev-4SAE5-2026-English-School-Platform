...
export class QuizService {
                ...
                switchMap(questions => {
                    ...
                    const requests = questions.map(q => {
                        ...
                        const answerPaths = [
                            ...
                            `/quiz-api/questions/${qId}/answers`,
    ...
    createQuiz(quiz: Quiz): Observable<Quiz> {
        const payload = { title: quiz.title, description: quiz.description, createdBy: quiz.createdBy || 1 };
        return this.http.post<Quiz>(this.quizzesUrl, payload, { headers: this.headers }).pipe(
            switchMap(newQuiz => {
                if (!quiz.questions || quiz.questions.length === 0) {
                    return of(newQuiz);
                }

                // Pour chaque question du quiz, on sérialise les options
                const questionReqs = quiz.questions.map(q => {
                    const optionsJson = q.answers ? JSON.stringify(q.answers) : '[]';
                    const qPayload = {
                        content: q.content,
                    ...
                    return this.http.post<QuizQuestion>(this.questionsUrl, qPayload, { headers: this.headers }).pipe(
                        map(createdQ => ({ ...createdQ, answers: q.answers })),
                        catchError(() => of({ ...q, id: Date.now() }))
                    );
                });

                return forkJoin(questionReqs).pipe(
                    map(questions => ({ ...newQuiz, questions: questions as QuizQuestion[] }))
                );
            }),
            tap(newQuiz => {
                ...
                this.quizzesSubject.next(this.mockQuizzes);
                this.saveToStorage();
                console.log('[QuizService] Quiz créé avec questions:', newQuiz);
            }),
            catchError(error => {
                console.error('[QuizService] Erreur création quiz:', error.status, error.error);
                const mockQuiz: Quiz = {
                    ...quiz,
                    id: Math.max(...this.mockQuizzes.map(q => q.id || 0), 0) + 1,
                    questions: quiz.questions || []
                };
                this.mockQuizzes.push(mockQuiz);
                this.quizzesSubject.next(this.mockQuizzes);
                this.saveToStorage();
                return of(mockQuiz);
            })
