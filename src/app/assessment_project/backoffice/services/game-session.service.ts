...
export class GameSessionService {
    ...
    createSession(quizId: number): Observable<GameSession> {
        return this.http.post<GameSession>(this.baseUrl, { quiz: { id: quizId } });
    }
    ...
    getPlayers(sessionId: number): Observable<string[]> {
        return this.http.get<string[]>(`${this.baseUrl}/${sessionId}/players`).pipe(
            catchError(() => of([]))
        );
    }
    ...
    getDetailedScores(sessionId: number): Observable<Score[]> {
        return this.http.get<Score[]>(`${this.scoresUrl}/session/${sessionId}/detailed`).pipe(
            catchError(() => of([]))
        );
    }
