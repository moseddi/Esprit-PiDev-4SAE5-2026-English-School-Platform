...
@Component({
  ...
  template: `
    <div class="kahoot-container" [class]="'phase-' + phase">
      ...
      <!-- 2. QUESTION PHASE -->
      <div *ngIf="phase === 'QUESTION' && currentQuestion" class="question-view">
  ...
  listenToWs(): void {
    this.wsSub = this.wsService.getEvents().subscribe((event: QuizEvent) => {
      ...
      if (event.type === 'START') {
        ...
        setTimeout(() => this.cdr.detectChanges(), 0);
        if (this.phase === 'LOBBY') this.startQuiz();
      }
  ...
  loadSession(id: number): void {
    this.sessionService.getSessionById(id).subscribe({
      next: (s) => {
        ...
        // Register in lobby — use response to immediately populate PIN and player list
        ...
        // If session already started before we joined, go straight to quiz
        const alreadyStarted = s.status === GameStatus.STARTED || s.status === GameStatus.IN_PROGRESS;
        if (alreadyStarted) {
          this.startQuiz();
          return;
        }
        // Poll every 2s for status changes (fallback for WS START event)
